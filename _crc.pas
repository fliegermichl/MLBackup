...
// 16 Bite CRC Funktionen
function Calc_CRC16(Data : PChar; Size : WORD; crc : WORD) : WORD; overload;
function Calc_CRC16(Data : PChar; Size : WORD) : WORD; overload;


// 32 Bit CRC Functions
function Calc_CRC32 (Data: PChar; Size: word; crc : longword) : longword; overload;
function Calc_CRC32 (Data: PChar; Size : word) : longword; overload;
...
function calc_crc32 (Data: PChar; Size: word; crc : longword) : longword;
var
 i   : word;
begin
 Result := crc;
 ...
 for i := 0 to Pred (Size) do
  crc := crc_32_tab[BYTE (crc XOR LONGWORD (Data[i]))] XOR ((crc SHR 8) AND $00FFFFFF);
 Result := crc
end;

function Calc_crc32(Data : PChar; Size : Word) : longword;
begin
 Result := $FFFFFFFF;
 if Size = 0 then exit;
 Result := Calc_crc32(Data, Size, Result);
end;

function Calc_CRC16(Data : PChar; Size : WORD; crc : WORD) : WORD;
var i : word;
begin
 Result := crc;
 if (not (Assigned(Data))) or (Size = 0) then exit;
 for i := 0 to Pred(Size) do
  crc := crctab[((crc SHR 8) AND 255)] XOR (crc SHL 8) XOR Byte(Data[i]);
 Result := crc;
end;

function Calc_CRC16(Data : PChar; Size : WORD) : WORD;
begin
 Result := Calc_crc16(Data, Size, 0);
end;
